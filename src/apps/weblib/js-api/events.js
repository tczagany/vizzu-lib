...
export default class Events
{
	...
	add(eventName, handler) {
		...
		try {
			let id = this.vizzu.call(this.module._addEventListener)
				(this.vizzu.chart, cname);
	...
	remove(eventName, handler) {
		...
		try {
			if (typeof handler === 'undefined') {
				this.vizzu.call(this.module._removeEventListener)
					(this.vizzu.chart, cname, 0);
			}
			else {
